resource "proxmox_vm_qemu" "virtual_machine_cloud_init" {
  ...
  dynamic "disk" {
    for_each = {
      for k, v in local.cloud_init_vms : k => v if v.disk_type != null && v.disk_storage_pool != null && v.disk_size != null
    }
    #local.cloud_init_vms && each.value.disk_type != null && each.value.disk_storage_pool != null && each.value.disk_size != null ? [1] : []
    content {
      ...
      mbps_wr = each.value.mbps_wr
      mbps_wr_max = each.value.mbps_wr_max
  ...
  dynamic "network" {
    for_each = {
      for k, v in local.cloud_init_vms : k => v if v.network_model != null
    }
    #local.cloud_init_vms && each.value.network_model != null ? [1] : []
    content {
